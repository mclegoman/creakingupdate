...
public class BirchCreakingHeartTreeDecorator extends TreeDecorator {
	public static final MapCodec<BirchCreakingHeartTreeDecorator> CODEC = Codec.floatRange(0.0F, 1.0F).fieldOf("probability").xmap(BirchCreakingHeartTreeDecorator::new, (treeDecorator) -> treeDecorator.probability);
	private final float probability;

	public BirchCreakingHeartTreeDecorator(float probability) {
			...
			if (!(random.nextFloat() >= this.probability)) {
				...
				Optional<BlockPos> optional = list2.stream().filter((pos) -> {
					...
					for (Direction direction : verticalDirections) {
						if (!generator.matches(pos.offset(direction), (state) -> state.isIn(BlockTags.BIRCH_LOGS) && state.get(PillarBlock.AXIS).isVertical())) return false;
					}
				...
				optional.ifPresent(blockPos -> generator.replace(blockPos, BlockRegistry.birchCreakingHeart.block().getDefaultState().with(OakCreakingHeartBlock.ACTIVE, true).with(OakCreakingHeartBlock.NATURAL, true)));
			}
		}
	}
}
