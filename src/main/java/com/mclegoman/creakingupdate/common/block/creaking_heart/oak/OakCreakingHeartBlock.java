...
public class OakCreakingHeartBlock extends CreakingVariantHeartBlock {
	public static final MapCodec<CreakingVariantHeartBlock> CODEC = createCodec(OakCreakingHeartBlock::new);
	public MapCodec<CreakingVariantHeartBlock> getCodec() {
		return CODEC;
	}
	public OakCreakingHeartBlock(AbstractBlock.Settings settings) {
		super(settings);
	...
	public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
		if (world.isClient) {
			...
			return state.get(ACTIVE) ? validateTicker(type, BlockEntityRegistry.oakCreakingHeart, CreakingVariantHeartBlockEntity::tick) : null;
		}
	}
	public static TagKey<Block> getLogs() {
		return BlockTags.OAK_LOGS;
	}
}