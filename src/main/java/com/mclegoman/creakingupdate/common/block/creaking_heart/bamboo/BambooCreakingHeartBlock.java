...
public class BambooCreakingHeartBlock extends CreakingVariantHeartBlock {
	public static final MapCodec<CreakingVariantHeartBlock> CODEC = createCodec(BambooCreakingHeartBlock::new);
	public MapCodec<CreakingVariantHeartBlock> getCodec() {
		return CODEC;
	}
	public BambooCreakingHeartBlock(Settings settings) {
		super(settings);
		this.setDefaultState(this.getDefaultState().with(AXIS, Direction.Axis.Y).with(ACTIVE, false).with(NATURAL, false));
	}
	public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
		return new BambooCreakingHeartBlockEntity(pos, state);
	}
	@Nullable
	public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
		if (world.isClient) {
			...
			return state.get(ACTIVE) ? validateTicker(type, BlockEntityRegistry.bambooCreakingHeart, CreakingVariantHeartBlockEntity::tick) : null;
		}
	}
	public static TagKey<Block> getLogs() {
		return BlockTags.BAMBOO_BLOCKS;
	}
}