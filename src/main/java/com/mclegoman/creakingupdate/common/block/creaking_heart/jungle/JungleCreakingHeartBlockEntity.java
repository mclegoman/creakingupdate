...
public class JungleCreakingHeartBlockEntity extends CreakingVariantHeartBlockEntity {
	...
	public static void tick(World world, BlockPos pos, BlockState state, JungleCreakingHeartBlockEntity blockEntity) {
		++blockEntity.ticks;
		if (world instanceof ServerWorld serverWorld) {
			...
			if (blockEntity.creakingUpdateTimer-- < 0) {
				...
				if (blockEntity.creakingPuppet == null) {
					if (!JungleCreakingHeartBlock.shouldBeEnabled(state, world, pos)) {
					...
					if (optional.isPresent()) {
						...
						if (!JungleCreakingHeartBlock.shouldBeEnabled(state, world, pos) && blockEntity.creakingPuppet == null) {
							world.setBlockState(pos, state.with(CreakingVariantHeartBlock.ACTIVE, false), 3);
						}
					}
